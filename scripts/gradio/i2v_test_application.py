...
class Image2Video():
    ...
    def get_image(self, image, prompt, steps=50, cfg_scale=7.5, eta=1.0, fs=3, seed=123, image2=None):
        ...
        unique_filename = f"video_{uuid.uuid4()}"

        save_videos(batch_samples, self.result_dir, filenames=[unique_filename], fps=self.save_fps)
        print(f"Saved in {unique_filename}. Time used: {(time.time() - start):.2f} seconds")
        model = model.cpu()
        return os.path.join(self.result_dir, f"{unique_filename}.mp4")
